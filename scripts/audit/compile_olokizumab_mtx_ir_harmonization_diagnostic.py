#!/usr/bin/env python3
"""Replay the exact olokizumab MTX-IR harmonization difficulty diagnostic."""
...
DEFAULT_RETRIEVED_AT = "2026-08-25T17:39:58.865108+00:00"
SOURCE_HASHES = {
    "NCT02760368": "72cd2ce3bb7a7b1dbd9a115b24e273959571a52187759528d8b05005a9dd299b",
    "NCT02760407": "854689f2a761cd428f05597b71594b28a7fa073db06991586aa560a6b4cf1a57",
}
...
def compile_report(
    nct02760368_source: Path,
    nct02760407_source: Path,
    retrieved_at: datetime,
):
    inventory_bundles = tuple(
        sorted(
            (
                _inventory("NCT02760368", nct02760368_source, retrieved_at),
                _inventory("NCT02760407", nct02760407_source, retrieved_at),
            ),
    ...
    candidate_spec = ClinicalTrialsGovHarmonizationCandidateSpec(
        packet_id=(f"olokizumab-mtx-ir-cross-trial-candidates:{REGISTRY_VERSION}"),
    ...
    diagnostic_spec = ClinicalTrialsGovHarmonizationDiagnosticSpec(
        report_id=(f"olokizumab-mtx-ir-harmonization-difficulty:{REGISTRY_VERSION}"),
...
def main() -> int:
    ...
    parser.add_argument("--nct02760368-source", required=True, type=Path)
    parser.add_argument("--nct02760407-source", required=True, type=Path)
    parser.add_argument(
        "--retrieved-at",
    ...
    if structure_requested or presence_requested:
        structure_spec = ClinicalTrialsGovHarmonizationStructureSpec(
            report_id=(f"olokizumab-mtx-ir-structure-decomposition:{REGISTRY_VERSION}"),
            candidate_packet_sha256=candidate_packet.fingerprint,
        ...
        if presence_requested:
            ...
            presence_spec = ClinicalTrialsGovHarmonizationPresenceSpec(
                report_id=(f"olokizumab-mtx-ir-presence-resolution:{REGISTRY_VERSION}"),
                candidate_packet_sha256=candidate_packet.fingerprint,
