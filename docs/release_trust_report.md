...
| Tokenizer transfer diagnostics retain failures after selection. | `agentic_drug_discovery/frontier_tokenizer_placebo.py`, `rl_env/specs/frontier_tokenizer_placebo_summary.json`, `tests/test_frontier_contract.py` | Frozen r50k/p50k evaluation-only profiles are loaded after cl100k/o200k selection. Aggregate L1 improves 10.4%, but four of 30 placebos regress and held-out exact token counts match only 4/60. The audit is post-selection, not preregistered or external, and the two 50k encodings are not treated as independent tokenizer families; robust generalization remains false. |
| Independent tokenizer-family evaluation is pre-sealed and failure-preserving. | `agentic_drug_discovery/frontier_tokenizer_independent_evaluation.py`, `rl_env/specs/frontier_tokenizer_independent_evaluation_protocol.json`, `rl_env/specs/frontier_tokenizer_independent_evaluation_summary.json`, `tests/test_frontier_contract.py` | Revision-pinned BERT WordPiece and T5 SentencePiece assets are hash- and version-checked after upstream selection. Aggregate L1 improves 10.9%, but seven of 30 placebos regress, profile non-regression reaches only 51/60, and exact optimized token counts match 1/60. The seal was local, not public or externally timestamped; no result changed upstream selection, all strict independent-family readiness fields remain false, and private text/assets/per-placebo diagnostics stay excluded. |
...
| Registry-record-wide outcome and safety inventory precedes endpoint selection. | `docs/54_clinicaltrials_gov_registry_record_inventory.md`, `agentic_drug_discovery/clinicaltrials_gov_inventory.py`, `rl_env/specs/clinicaltrials_gov_inventory_spec.schema.json`, `rl_env/specs/clinicaltrials_gov_inventory_packet.schema.json`, `tests/test_clinicaltrials_gov_inventory.py` | One exact NCT/version/source receipt is replayed across every protocol primary/secondary/other outcome, posted outcome, safety group, serious/other event, and group-level event statistic present in the source arrays. Source order, JSON pointers, hashes, structural counts, absent modules, unmatched outcomes, and ambiguous exact lexical candidates remain visible. Synthetic controls cover completeness, source drift, pointer/count rebinding, strict JSON, schemas, CLI, and wheel execution. No real inventory, endpoint selection, semantic equivalence, clinical comparability, causal safety relation, synthesis, or treatment choice is claimed. |
| Cross-trial harmonization begins from a complete bounded reviewer universe. | `docs/55_cross_trial_harmonization_candidates.md`, `agentic_drug_discovery/clinicaltrials_gov_harmonization_candidates.py`, `rl_env/specs/clinicaltrials_gov_harmonization_candidate_spec.schema.json`, `rl_env/specs/clinicaltrials_gov_harmonization_candidate_packet.schema.json`, `tests/test_clinicaltrials_gov_harmonization_candidates.py` | Exact inventory fingerprints and source-disjoint receipts/hashes bind the input set. A preflight computes the full cross-trial posted-outcome Cartesian size before allocation; every pair, missing field, within-trial lexical status, and trial-level safety record reference remains visible, including trials with no posted outcomes. Synthetic controls cover 2x3 enumeration, zero-result retention, ambiguity, missingness, safety-window disagreement, bounds, strict JSON, schemas, replay, CLI, and rehashed tampering. Mechanical equality does not approve endpoint identity, estimand equivalence, clinical or safety comparability, synthesis, or treatment choice. |
| Harmonization difficulty is measurable without publishing pair payloads or granting semantic approval. | `docs/56_cross_trial_harmonization_difficulty_diagnostics.md`, `docs/ra_olokizumab_mtx_ir_harmonization_diagnostic_spec.json`, `docs/ra_olokizumab_mtx_ir_harmonization_diagnostic_report.json`, `agentic_drug_discovery/clinicaltrials_gov_harmonization_diagnostics.py`, `tests/test_clinicaltrials_gov_harmonization_diagnostics.py` | The compiler partitions every complete-graph pair into explicit field/structure denominators, overlapping review routes, and mutually exclusive blocker signatures. The exact NCT02760368/NCT02760407 run covers 35 pairs and publishes only NCT/source/inventory hashes and aggregate counts. Raw snapshots, inventories, candidate graph, endpoint titles, pair IDs, safety terms, and semantic decisions remain external. Counts quantify review workload, not endpoint equivalence, model accuracy, efficacy, comparative safety, benefit-risk, or treatment choice. |
| Cross-disease harmonization robustness preserves cohort denominators and avoids ranking. | `docs/57_cross_disease_harmonization_robustness.md`, `docs/uc_ozanimod_harmonization_diagnostic_report.json`, `docs/ra_uc_harmonization_robustness_spec.json`, `docs/ra_uc_harmonization_robustness_report.json`, `agentic_drug_discovery/clinicaltrials_gov_harmonization_robustness.py`, `tests/test_clinicaltrials_gov_harmonization_robustness.py` | An exact 10x11 ozanimod UC graph replicates the payload-free diagnostic on 110 pairs. The source-disjoint RA/UC compiler retains each rational numerator/denominator and labels route, missingness, and structural prevalence as saturated, absent, stable non-boundary, or heterogeneous. Three review routes saturate both cohorts, two are absent in both, and five vary. Report hashes, NCT disjointness, strict JSON, schemas, CLI replay, no-pooling, no-weighting, no-ranking, and no-generalization flags fail closed. This is mechanical robustness profiling, not endpoint equivalence, semantic accuracy, disease generalization, efficacy, safety, benefit-risk, or treatment choice. |
| Structural disagreement causes are separated without discounting trial-wide differences. | `docs/58_structural_disagreement_decomposition.md`, `docs/ra_olokizumab_mtx_ir_structure_report.json`, `docs/uc_ozanimod_structure_report.json`, `agentic_drug_discovery/clinicaltrials_gov_harmonization_structure.py`, `tests/test_clinicaltrials_gov_harmonization_structure.py` | Every one of seven prior structural diagnostics is refined into exact, trial-global, and endpoint-local pair counts under exact candidate/diagnostic hash binding. The same saturated group-count rate is trial-global for all 35 RA pairs but endpoint-local for all 110 UC pairs, showing that aggregate route stability can conceal different causes. Reports retain only aggregate trial profiles and cause counts. Source-array absence versus present-empty remains non-identifiable in v1; trial-global differences are never discounted. This is a representation diagnostic, not semantic or estimand equivalence, clinical comparability, disease generalization, efficacy, safety, benefit-risk, or treatment choice. |
| Structural array states preserve absence, null, empty, and non-empty provenance. | `docs/59_presence_preserving_structural_arrays.md`, `docs/ra_olokizumab_mtx_ir_presence_report.json`, `docs/uc_ozanimod_presence_report.json`, `agentic_drug_discovery/clinicaltrials_gov_structural_presence.py`, `tests/test_clinicaltrials_gov_structural_presence.py` | Exact source replay emits private per-pointer sidecars and public payload-free aggregate reports. The UC replay resolves 192 formerly ambiguous field-pairs while the RA replay confirms zero legacy ambiguity. Raw source bytes, sidecars, endpoint values, and semantic decisions remain external. Presence state is representation provenance, not source completeness, endpoint identity, estimand equivalence, comparability, efficacy, safety, benefit-risk, or treatment choice. |
| Endpoint/estimand review readiness is preregistered and source-bound without semantic approval. | `docs/60_preregistered_endpoint_estimand_review_preflight.md`, the four adjacent exact RA/UC spec and packet artifacts, `agentic_drug_discovery/clinicaltrials_gov_endpoint_estimand_preflight.py`, `tests/test_clinicaltrials_gov_endpoint_estimand_preflight.py` | A fixed ICH E9(R1)-aligned five-dimension contract binds exact candidate, presence, sidecar, inventory, registry-version, retrieval-time, locator, receipt, and source-content hashes. On the 2026-08-27 snapshots, all 35 RA pairs and 14/110 UC pairs are structurally ready for human review; 96 UC pairs require source completion because one or both registry outcomes omit analyses. Missing dispersion is retained as nonblocking context in 34 RA and all 110 UC pairs. Every estimand dimension remains unresolved, no pair is automatically excluded, and no endpoint identity, equivalence, comparability, pooling, efficacy, safety, benefit-risk, or treatment choice is approved. |
...
| Multi-endpoint portfolios expose evidence reuse without manufacturing independence. | `docs/51_multi_endpoint_benefit_risk_portfolio.md`, `agentic_drug_discovery/clinical_benefit_risk_portfolio.py`, `rl_env/specs/clinical_benefit_risk_portfolio_spec.schema.json`, `rl_env/specs/clinical_benefit_risk_portfolio_report.schema.json`, `tests/test_clinical_benefit_risk_portfolio.py` | Every selected synthesis is replayed from one committed state and remains a separate endpoint domain. Endpoint, trial, safety, and source reference counts are paired with unique-unit and reuse counts. Safety units retain exact fingerprints, time frames, raw counts, observed risks, and every referencing domain. Strict mode rejects endpoint-record relabeling; diagnostic mode retains and quantifies it. Synthetic four-trial and overlap controls validate the contract only. No real multi-endpoint result, cross-endpoint pooling, scalar benefit-risk score, safety independence, comparability, clinical acceptability, or treatment choice is claimed. |
| Same-trial endpoint pairs expose population and window heterogeneity without inferring comparability. | `docs/52_same_trial_multi_endpoint_stress.md`, `agentic_drug_discovery/clinical_benefit_risk_portfolio_stress.py`, `rl_env/specs/clinical_benefit_risk_portfolio_stress_report.schema.json`, `tests/test_clinical_benefit_risk_portfolio_stress.py` | Every unordered endpoint-domain pair represented in one trial is replayed against exact endpoint, population, phase, safety, and source ledgers. Matched, phase-undeclared, and heterogeneous structures retain separate diagnostic codes; shared safety and source artifacts remain reuse rather than independent observations. Identity-closed two-trial controls separate population and endpoint-window mismatches and retain endpoint relabeling. No real pairwise result, shared estimand, participant identity, endpoint exchangeability, clinical comparability, safety independence, or treatment choice is claimed. |
| Pre-review endpoint candidates are exhaustive and provenance-bound without semantic approval. | `docs/53_provenance_bound_endpoint_review_candidates.md`, `agentic_drug_discovery/clinical_endpoint_review_candidates.py`, `rl_env/specs/clinical_endpoint_review_candidate_spec.schema.json`, `rl_env/specs/clinical_endpoint_review_candidate_packet.schema.json`, `tests/test_clinical_endpoint_review_candidates.py` | Every population, endpoint, and safety record from an exact design set remains in a candidate or mechanically excluded partition. All eligible same-design endpoint pairs and endpoint-by-serious-safety links are enumerated with record/source hashes, phase, population, window, arm, and raw safety diagnostics. Missing provenance, omission, rebinding, unknown fields, duplicate keys, rehashed counts, and semantic-approval flags fail closed. Synthetic controls validate complete secondary-endpoint retention and exclusion behavior only. No real packet, reviewer decision, endpoint family, ontology mapping, shared estimand, comparability, safety relationship, synthesis, or treatment choice is claimed. |
...
| `docs/57_cross_disease_harmonization_robustness.md` | Human-readable exact UC replication, RA/UC rational-rate stability, structural confounding warning, and next research priority. |
| `docs/uc_ozanimod_harmonization_diagnostic_spec.json` | Exact UC candidate-graph hash and fixed payload-free diagnostic policy. |
| `docs/uc_ozanimod_harmonization_diagnostic_report.json` | Integrity-bound UC source/inventory hashes, 110-pair aggregate routes/signatures, and nonclaims. |
| `docs/ra_uc_harmonization_robustness_spec.json` | Canonical source-disjoint RA/UC diagnostic-report hashes. |
| `docs/ra_uc_harmonization_robustness_report.json` | Exact per-cohort rates, stability labels, integrity, and fixed no-pooling/no-ranking flags. |
| `docs/58_structural_disagreement_decomposition.md` | Human-readable exact RA/UC structural-cause analysis, source-presence limit, hashes, and next research priority. |
| `docs/ra_olokizumab_mtx_ir_structure_spec.json` | Exact RA candidate/diagnostic hashes and fixed structural-decomposition policy. |
...
| `docs/60_preregistered_endpoint_estimand_review_preflight.md` | Fixed five-dimension review contract, temporal source boundary, exact RA/UC routes, replay, and nonapproval interpretation. |
| `docs/ra_olokizumab_mtx_ir_endpoint_estimand_preflight_spec.json` | Exact 2026-08-27 RA candidate, presence, sidecar, and pair-bound preflight policy. |
| `docs/ra_olokizumab_mtx_ir_endpoint_estimand_preflight_packet.json` | Payload-free RA source identities, 35 pair routes, context gaps, and unresolved estimand dimensions. |
| `docs/uc_ozanimod_endpoint_estimand_preflight_spec.json` | Exact 2026-08-27 UC candidate, presence, sidecar, and pair-bound preflight policy. |
| `docs/uc_ozanimod_endpoint_estimand_preflight_packet.json` | Payload-free UC source identities, 110 pair routes, typed source blockers, context gaps, and unresolved estimand dimensions. |
...
| `agentic_drug_discovery/clinicaltrials_gov_inventory.py` | Exact registry-record outcome/result/safety enumeration, source pointers and hashes, two-sided lexical candidate diagnostics, strict integrity readers, and fixed no-selection/nonclaim fields. |
| `agentic_drug_discovery/clinicaltrials_gov_harmonization_candidates.py` | Exact-inventory input binding, source-disjoint trial safety contexts, bounded full-Cartesian endpoint pairs, explicit missingness, strict replay, and fixed semantic nonclaims. |
...
| `agentic_drug_discovery/clinicaltrials_gov_endpoint_estimand_preflight.py` | Exact candidate/presence/sidecar binding, fixed five-dimension pair routes, blocker/context-gap separation, strict replay, and no automatic exclusion or semantic approval. |
...
| `agentic_drug_discovery/clinical_endpoint_review_candidates.py` | Exhaustive pre-review record partitions, pair/link candidate enumeration, strict provenance binding, integrity readers, and fixed semantic nonclaims. |
...
| `rl_env/specs/clinical_endpoint_review_candidate_spec.schema.json` | Machine schema for exact pre-review candidate/intervention/disease/design scope. |
| `rl_env/specs/clinical_endpoint_review_candidate_packet.schema.json` | Machine schema for record partitions, pair/link candidates, provenance, counts, integrity, and no-approval flags. |
| `rl_env/specs/clinicaltrials_gov_inventory_spec.schema.json` | Machine schema for one exact source receipt, NCT, version, inventory identity, and fixed record-wide policy. |
| `rl_env/specs/clinicaltrials_gov_inventory_spec.example.json` | Synthetic registry-record inventory declaration with no source bytes, reviewer decision, or clinical claim. |
| `rl_env/specs/clinicaltrials_gov_inventory_packet.schema.json` | Machine schema for complete source-array inventories, lexical links, module presence, pointers, hashes, counts, integrity, and nonclaims. |
| `rl_env/specs/clinicaltrials_gov_harmonization_candidate_spec.schema.json` | Machine schema for canonical exact-inventory bindings, fixed policy, and bounded pair count; the adjacent example is synthetic. |
| `rl_env/specs/clinicaltrials_gov_harmonization_candidate_packet.schema.json` | Machine schema for safety contexts, posted endpoints, full cross-trial pair diagnostics, missingness, integrity, and fixed nonclaims. |
| `rl_env/specs/clinicaltrials_gov_harmonization_diagnostic_spec.schema.json` | Machine schema for exact candidate-packet binding and the fixed payload-free diagnostic policy; the adjacent example is synthetic. |
| `rl_env/specs/clinicaltrials_gov_harmonization_diagnostic_report.schema.json` | Machine schema for aggregate trial, field, structure, route, signature, Cartesian-count, integrity, and fixed nonclaim diagnostics. |
| `rl_env/specs/clinicaltrials_gov_harmonization_robustness_spec.schema.json` | Machine schema for canonical source-disjoint cohort/report bindings and exact hashes. |
| `rl_env/specs/clinicaltrials_gov_harmonization_robustness_report.schema.json` | Machine schema for exact cohort rates, stability labels, source-disjointness, integrity, and fixed no-pooling/no-ranking flags. |
| `rl_env/specs/clinicaltrials_gov_harmonization_structure_spec.schema.json` | Machine schema for exact candidate/diagnostic binding and the fixed decomposition policy; the adjacent example is synthetic. |
| `rl_env/specs/clinicaltrials_gov_harmonization_structure_report.schema.json` | Machine schema for trial profiles, cause partitions, source-presence limits, integrity, and fixed nonclaims. |
| `rl_env/specs/clinicaltrials_gov_structural_presence_spec.schema.json` | Machine schema for one exact inventory/source binding and bounded ten-role array-state extraction; the adjacent example is synthetic. |
| `rl_env/specs/clinicaltrials_gov_structural_presence_packet.schema.json` | Machine schema for private per-pointer absent/null/empty/non-empty provenance and integrity. |
| `rl_env/specs/clinicaltrials_gov_harmonization_presence_spec.schema.json` | Machine schema for exact candidate, structure, inventory, and sidecar bindings; the adjacent example is synthetic. |
| `rl_env/specs/clinicaltrials_gov_harmonization_presence_report.schema.json` | Machine schema for public aggregate array-role profiles and legacy-ambiguity resolution. |
| `rl_env/specs/clinicaltrials_gov_endpoint_estimand_preflight_spec.schema.json` | Machine schema for exact candidate/presence/sidecar bindings and the fixed five-dimension review policy; the adjacent example is synthetic. |
| `rl_env/specs/clinicaltrials_gov_endpoint_estimand_preflight_packet.schema.json` | Machine schema for exact source identities, every payload-free pair route, blocker/context-gap counts, unresolved dimensions, integrity, and fixed nonapprovals. |
...
| `tests/test_clinicaltrials_gov_inventory.py` | Complete protocol/posted/safety array retention, two-sided lexical partitions, module absence, strict schemas/readers, source/tamper controls, CLI, and wheel coverage. |
| `tests/test_clinicaltrials_gov_harmonization_candidates.py` | Full 2x3 cross-trial pair enumeration, zero-result retention, ambiguity/missingness/safety diagnostics, bounds, source-disjointness, schemas, strict readers, replay, CLI, and tamper controls. |
| `tests/test_clinicaltrials_gov_harmonization_diagnostics.py` | Aggregate partitions, explicit denominators, overlapping routes, Cartesian invariants, payload exclusion, strict public artifact, schemas, CLI, replay, and rehashed tamper controls. |
| `tests/test_clinicaltrials_gov_harmonization_robustness.py` | Exact RA/UC rates, input-order determinism, report-hash and NCT-overlap rejection, strict public schemas/readers, CLI replay, and payload exclusion. |
| `tests/test_clinicaltrials_gov_harmonization_structure.py` | Exact cause partitions, global/local disagreement separation, strict public artifacts, schemas, replay, CLI, payload exclusion, and tamper controls. |
| `tests/test_clinicaltrials_gov_structural_presence.py` | Ten-role source-state extraction, aggregate ambiguity resolution, exact public artifacts, strict schemas/readers, replay, CLI, and source-tamper controls. |
| `tests/test_clinicaltrials_gov_endpoint_estimand_preflight.py` | Missing-analysis and null-group blockers, nonblocking dispersion context, identity routes, fixed unresolved dimensions, exact public artifact hashes/source identities, strict schemas/readers, generic cohort replay, source-byte bounds, payload exclusion, and CLI controls. |
...
| `scripts/audit/compile_olokizumab_mtx_ir_harmonization_diagnostic.py` | Exact external-source hash verification plus inventory, complete candidate-graph, and payload-free diagnostic replay for the public olokizumab aggregate. |
| `scripts/audit/compile_ozanimod_uc_harmonization_diagnostic.py` | Exact external-source hash verification plus inventory, complete 110-pair candidate graph, and payload-free diagnostic replay for the public ozanimod UC aggregate. |
| `scripts/audit/compile_clinicaltrials_gov_endpoint_estimand_preflight.py` | Source-byte- and work-bounded exact-cohort replay from externally pinned registry bytes through inventory, candidate, diagnostic, structure, sidecars, aggregate presence, and payload-free endpoint/estimand preflight artifacts. |
