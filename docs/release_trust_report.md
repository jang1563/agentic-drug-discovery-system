...
| A second disease separates synthetic breadth from public-source provider execution. | `docs/41_ulcerative_colitis_conformance_slice.md`, `docs/42_uc_provider_validation.md`, `docs/uc_clinical_provider_validation_snapshot.json`, `tests/test_cross_disease_clinical_conformance.py`, `tests/test_clinicaltrials_gov_ingestion.py` | The synthetic UC fixture preserves `MONDO:0005101` from contextual handoff through non-pooled synthesis. Separately, source-pinned `NCT01647516` and `NCT02435992` induction records preserve exact ratio intervals, arm order, treatment phase, and safety aggregates: the uncertain Phase 2 interval commits on `HOLD`, and the bounded favorable Phase 3 interval advances. Raw bytes and jobs remain external. This validates provider contract behavior, not pooled efficacy, safety acceptability, independent scientific review, or a second end-to-end therapeutic slice. |
