...
| A second disease exercises the cross-disease contract without being overstated as validation. | `docs/41_ulcerative_colitis_conformance_slice.md`, `rl_env/specs/translational_handoff.uc.synthetic.example.json`, `rl_env/specs/clinical_endpoint_mapping.uc.synthetic.example.json`, `rl_env/specs/clinical_benefit_risk_synthesis.uc.synthetic.example.json`, `tests/test_cross_disease_clinical_conformance.py` | The UC fixture preserves `MONDO:0005101` across contextual handoff, clinical-remission mapping, higher-is-better odds-ratio synthesis, and evidence-tensor compilation. All perturbations, targets, trial ids, endpoint values, sources, and reviewers are synthetic. This proves contract conformance only, not real-source ingestion, independent scientific review, efficacy, safety, or a second validated vertical slice. |
