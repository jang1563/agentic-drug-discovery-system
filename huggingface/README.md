...
The registry-harmonization path now includes a preregistered endpoint/estimand review preflight.
It binds exact candidate, presence, and sidecar fingerprints; retains payload-free pair/source
identities; and routes missing source structure separately from endpoint-identity reconciliation.
On the exact 2026-08-27 snapshots, all 35 RA pairs are review-ready, while 96 of 110 UC pairs
require source completion because at least one exact `analyses` array is absent and 14 are
review-ready. Missing dispersion is retained as a nonblocking context gap. Ready does not mean
equivalent: treatment, population, variable, intercurrent-event strategy, and population-level
summary remain unresolved for every pair, and no human review was performed.

The follow-on evidence-acquisition stage verifies all six registry-declared protocol/SAP files
(693 pages; 21,372,435 bytes) without mirroring the PDFs. It normalizes the 49 review-ready pairs
to 21 unique endpoints and 105 endpoint-dimension records with 509 payload-free page cues. All
records have at least one lexical cue, but evidence sufficiency, contradictions, and estimand
equivalence remain unassessed; the 96 UC source-blocked pairs remain blocked.
...
- Inspect `docs/51_multi_endpoint_benefit_risk_portfolio.md` and
  `tests/test_clinical_benefit_risk_portfolio.py` for full-state-replayed endpoint domains,
  deduplicated safety/source units, strict endpoint-record distinctness, diagnostic reuse counts,
  integrity envelopes, and the no-pooling/no-score boundary.
- Inspect `docs/52_same_trial_multi_endpoint_stress.md` and
...
- Inspect `docs/53_provenance_bound_endpoint_review_candidates.md` and
  ...
  boundaries.
- Inspect `docs/54_clinicaltrials_gov_registry_record_inventory.md` and
  `tests/test_clinicaltrials_gov_inventory.py` for exact source-record enumeration across protocol
  primary/secondary/other outcomes, posted outcomes, serious/other events, source pointers/hashes,
  two-sided unmatched/ambiguous lexical candidates, strict replay, CLI, and no-selection
  boundaries.
- Inspect `docs/55_cross_trial_harmonization_candidates.md` and
  `tests/test_clinicaltrials_gov_harmonization_candidates.py` for bounded full-Cartesian
  posted-outcome review across exact trial inventories, zero-result trial retention, source-bound
  safety contexts, explicit field missingness, strict replay, CLI, and no-comparability boundaries.
- Inspect `docs/56_cross_trial_harmonization_difficulty_diagnostics.md`, its two adjacent
  machine-readable olokizumab artifacts, and
...
- Inspect `docs/58_structural_disagreement_decomposition.md`, its four adjacent RA/UC machine
...
- Inspect `docs/60_preregistered_endpoint_estimand_review_preflight.md`, its four exact RA/UC
  machine artifacts, and `tests/test_clinicaltrials_gov_endpoint_estimand_preflight.py` for fixed
  five-dimension review routing, temporal source bindings, nonblocking context gaps, and the hard
  no-approval boundary.
- Inspect `docs/61_source_bound_estimand_evidence_acquisition.md`, its four exact RA/UC machine
  artifacts, and `tests/test_clinicaltrials_gov_estimand_evidence_acquisition.py` for complete
  protocol/SAP inventory verification, bounded payload-free page cues, unchanged preflight
  blockers, byte-tamper rejection, and the no-semantic-sufficiency boundary.
...
| `docs/54_clinicaltrials_gov_registry_record_inventory.md` | Registry-record-wide protocol/result/safety enumeration, exact lexical candidate partitions, source pointers/hashes, replay, and no-selection boundary. |
| `docs/55_cross_trial_harmonization_candidates.md` | Complete bounded cross-trial posted-outcome Cartesian review graph, source-disjoint safety contexts, missingness diagnostics, replay, and no-comparability boundary. |
...
| `docs/58_structural_disagreement_decomposition.md` | Exact RA/UC trial-global versus endpoint-local decomposition, source-presence limit, and replay contract. |
| `docs/ra_olokizumab_mtx_ir_structure_spec.json` | Exact RA candidate/diagnostic binding and fixed decomposition policy. |
...
| `docs/60_preregistered_endpoint_estimand_review_preflight.md` | Fixed five-dimension protocol, temporal snapshot boundary, exact RA/UC routes, replay, and interpretation limits. |
...
| `docs/ra_olokizumab_mtx_ir_endpoint_estimand_preflight_packet.json` | Payload-free RA source identities, pair routes, context gaps, and unresolved estimand dimensions. |
...
| `docs/uc_ozanimod_endpoint_estimand_preflight_packet.json` | Payload-free UC source-completion and review-ready routes with fixed nonapprovals. |
| `docs/61_source_bound_estimand_evidence_acquisition.md` | Exact six-document acquisition contract, five-dimension page-cue policy, RA/UC results, and semantic limits. |
| `docs/ra_olokizumab_mtx_ir_estimand_evidence_acquisition_spec.json` | Exact RA candidate/preflight and two combined protocol/SAP byte bindings. |
| `docs/ra_olokizumab_mtx_ir_estimand_evidence_acquisition_packet.json` | Payload-free RA document provenance, 60 endpoint-dimension records, and 300 page cues. |
| `docs/uc_ozanimod_estimand_evidence_acquisition_spec.json` | Exact UC candidate/preflight and four protocol/SAP byte bindings. |
| `docs/uc_ozanimod_estimand_evidence_acquisition_packet.json` | Payload-free UC provenance, 45 evidence records, 209 page cues, and 96 preserved blockers. |
...
| `agentic_drug_discovery/clinicaltrials_gov_endpoint_estimand_preflight.py` | Exact candidate/presence/sidecar binding, fixed five-dimension pair routes, blocker/context-gap separation, strict replay, and no automatic exclusion or approval. |
| `agentic_drug_discovery/clinicaltrials_gov_estimand_evidence_acquisition.py` | Complete protocol/SAP inventory and byte verification, bounded five-dimension page cues, normalized pair references, strict replay, and no excerpts or semantic approval. |
...
| `rl_env/specs/clinicaltrials_gov_inventory_spec.schema.json` | Exact source receipt, NCT, registry version, inventory identity, and fixed record-wide policy. |
| `rl_env/specs/clinicaltrials_gov_inventory_spec.example.json` | Synthetic exact-study inventory declaration without source bytes or a clinical claim. |
| `rl_env/specs/clinicaltrials_gov_inventory_packet.schema.json` | Complete source-array records, module presence, lexical links, pointers, hashes, counts, integrity, and no-selection fields. |
| `rl_env/specs/clinicaltrials_gov_harmonization_candidate_spec.schema.json` | Exact inventory fingerprints, canonical multi-trial scope, fixed policy, and pair-count bound; the adjacent example is synthetic. |
...
| `rl_env/specs/clinicaltrials_gov_endpoint_estimand_preflight_spec.schema.json` | Exact candidate/presence/sidecar and fixed five-dimension preregistration; the adjacent example is synthetic. |
| `rl_env/specs/clinicaltrials_gov_endpoint_estimand_preflight_packet.schema.json` | Source identities, every pair route, blocker/context-gap counts, unresolved estimand dimensions, integrity, and fixed nonapprovals. |
| `rl_env/specs/clinicaltrials_gov_estimand_evidence_acquisition_spec.schema.json` | Exact candidate/preflight and protocol/SAP bindings, page bound, and fixed anchor policy; the adjacent example is synthetic. |
| `rl_env/specs/clinicaltrials_gov_estimand_evidence_acquisition_packet.schema.json` | Verified documents, ranked payload-free page cues, normalized endpoint/pair references, integrity, and fixed nonapprovals. |
...
| `tests/test_clinicaltrials_gov_harmonization_structure.py` | Trial-global/local separation, diagnostic refinement, strict schemas/readers, exact RA/UC artifacts, CLI, and tamper controls. |
| `tests/test_clinicaltrials_gov_endpoint_estimand_preflight.py` | Absent-analysis, null-group-ID, nonblocking-dispersion, identity-route, exact public artifact/source binding, source-byte-bounded cohort replay, strict reader/schema, payload, and CLI controls. |
| `tests/test_clinicaltrials_gov_estimand_evidence_acquisition.py` | Complete document binding, PDF tamper rejection, preflight-block preservation, bounded cues, strict schemas/readers, exact public artifacts, payload exclusion, and replay. |
