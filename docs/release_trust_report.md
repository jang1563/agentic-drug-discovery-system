...
| Population-stratified transport diagnostics fail closed at estimability. | `docs/47_ra_olokizumab_population_stratified_transport.md`, `docs/ra_olokizumab_population_transport_report.json`, `agentic_drug_discovery/clinical_population_transport.py`, `rl_env/specs/clinical_population_transport_spec.schema.json`, `rl_env/specs/clinical_population_transport_report.schema.json`, `tests/test_clinical_benefit_risk_synthesis.py` | The exact olokizumab synthesis and source bytes replay into reviewed methotrexate- and TNF-inhibitor-inadequate-response strata. Trial effects remain side by side with no pooling or cross-stratum contrast. A target population, same-stratum replication, individual-level covariates, preregistered transport model, and risk-of-bias assessment are absent, so no transport effect is emitted. Source, population, field, and report hashes fail closed under rebinding or tampering. This is descriptive project-internal stratification, not independent review, causal transport, comparative safety, clinical acceptability, or treatment choice. |
